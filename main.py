...
async def on_ready():
    ...
    while True:
        ...
        activity = discord.Activity(type=discord.ActivityType.watching, name=f'Shop Good')
...
async def bronze(ctx):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    embed=discord.Embed(title="Free Bronze",color=10181046, description=f"**set .gg/EDebSEpnJD status, you will automatically get a rank Bronze**")
    await ctx.send(embed=embed)
...
async def on_member_update(before, after):
    role_id = 972615836214886514
    role = get(before.guild.roles, id=role_id)
    if 'https://discord.gg/EDebSEpnJD' in str(before.activities):
      if 'https://discord.gg/EDebSEpnJD' in str(after.activities):
        pass
      else:
        await after.remove_roles(role)
        channel = bot.get_channel(972615836617560154)
...
@bot.command()
async def clear(ctx):
    webhook = DiscordWebhook(url='https://discord.com/api/webhooks/974385892955389962/GZBYLr0efevs1d0UQenoy3VLYBFPEwR57FTw65KgvoARaqsalNsJvA7CuqOFNYrmBC3A')
    embed = DiscordEmbed(title='Shop Good', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}clear**'), color='9B59B6')
...
async def tspam(ctx, arg1, *, args):
 if ctx.channel.type != discord.ChannelType.private:
    config_file = get_config()
    json_object = json.loads(config_file)
    webhook = DiscordWebhook(url='https://discord.com/api/webhooks/https://discord.com/api/webhooks/974385892955389962/GZBYLr0efevs1d0UQenoy3VLYBFPEwR57FTw65KgvoARaqsalNsJvA7CuqOFNYrmBC3A')
